...
material_instance_init :: proc() -> bool {
...
material_instance_update_dirty_materials :: proc() {
	for i in 0 ..< G_MATERIAL_INSTANCE_REF_ARRAY.alive_count {
		...
		material_instance := &g_resources.material_instances[material_instance_get_idx(material_instance_ref)]
...
material_instance_create :: proc(p_material_instance_ref: MaterialInstanceRef) -> bool {
	material_instance := &g_resources.material_instances[material_instance_get_idx(p_material_instance_ref)]
	material_instance.flags += {.Dirty}
	return true
}
...
material_instance_allocate :: proc(p_name: common.Name) -> MaterialInstanceRef {
...
material_instance_get_idx :: proc(p_ref: MaterialInstanceRef) -> u32 {
	return common.ref_get_idx(&G_MATERIAL_INSTANCE_REF_ARRAY, p_ref)
}

//--------------------------------------------------------------------------//

material_instance_destroy :: proc(p_ref: MaterialInstanceRef) {
...
material_instance_get_properties_ptr :: proc(
	...
	return &INTERNAL.material_properties_array[material_instance_get_idx(p_material_instance_ref)]
}
